...
sys.path.insert(0, "../common_file")  # XD

from parallel import BalancedDataParallel
import tokenization
from modeling_xd import BertConfig, BertForSequenceClassification
...
class DataProcessor(object):
    ...
    def read_novel_examples(self, path, top_n=5, task_name='train'):
        ...
        rand_top_n = top_n == -1  # XD
        with open(path, 'r', encoding='utf-8') as f:
            for _, line in enumerate(f):
                if rand_top_n:  # XD
                    top_n = random.choice([3, 5, 7, 9, 11]) if task_name == 'train' else 7
                line = line.replace('\n', '').split('\t')
                text_a = line[0].split('||')
                ...
                try:
                    ...
                    # XD print(f'line = {line}')
                    # XD print(f'text_a = {text_a}')
                    continue
                ...
                middle = top_n // 2
                text_a = [''.join(text_a[:middle]), text_a[middle] + ''.join(text_a[middle + 1:])]  # XD
...
def main():
    ...
    if os.path.exists(args.output_dir) and os.listdir(args.output_dir) and args.do_train and False:  # XD
    ...
    def accuracy(example_ids, logits, probs=None, data_type='eval'):
        ...
        result = classification_report(labels, logits, target_names=classify_name, digits=3)
        return result
    ...
    # XD for k, v in model.state_dict().items():
    # XD     print(f'k = {k}, v shape {v.shape}')
    ...
    no_decay = ['bias', 'gamma', 'beta'] + ['LayerNorm.weight']  # XD
    ...
    if args.do_train:
        ...
        # XD eval_loss, acc, _ = eval_model(model, eval_dataloader, device)
        # XD logger.info(f'初始开发集loss: {eval_loss}')
