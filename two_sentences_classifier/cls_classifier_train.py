    ...
    def read_novel_examples(self, path, top_n=7, task_name='train'):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for _, line in enumerate(f):
                ...
                assert len(paras) == 2
                text_a = paras[0].split('||')[:top_n]
                text_b = paras[1].split('||')[:top_n]
                text_a = ''.join(text_a)
                text_b = ''.join(text_b)
        ...
        print(f'zero = {zero}, one = {one}  add_one_data = {add_one_data} \n')
...
def convert_examples_to_features(examples, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if length <= 300:
            sent_length_counter['<300'] += 1
        elif 300 < length <= 600:
            sent_length_counter['600>300'] += 1
        elif 600 < length <= 1000:
            sent_length_counter['1000>600'] += 1
        elif 1000 < length <= 1300:
            sent_length_counter['1300>1000'] += 1
        else:
            sent_length_counter['>1300'] += 1

        sents_tokens = sents_tokens[:max_seq_length - 1] + ['[SEP]']
        ...
        assert len(input_masks) == len(input_ids) == len(segment_ids) == len(position_ids)
        features.append(
...
def main():
    ...
    def eval_model(model, eval_dataloader, device, data_type='eval'):
        ...
        for step, batch in enumerate(tqdm(eval_dataloader, desc="evaluating")):
            example_ids, input_ids, input_mask, segment_ids, position_ids, label_ids = batch
