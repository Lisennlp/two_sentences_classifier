    ...
    def eval_model(model, eval_dataloader, device):
        ...
        for step, batch in enumerate(tqdm(eval_dataloader, desc="evaluating")):
            ...
            with torch.no_grad():
                ...
                second_labels = label_ids[first_labels == 1]
                second_example_ids = example_ids[first_labels == 1]
                if second_labels.sum():
                    second_labels -= 1
                    second_argmax_logits = torch.argmax(logits[1], dim=1)
                first_argmax_logits = torch.argmax(logits[0], dim=1)
                # first_indices = torch.arange(first_argmax_logits.size()[0])
                # logits_probs = logits[0][first_indices, first_argmax_logits]
            if args.do_train:
