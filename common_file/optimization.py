...
class _LRSchedule(ABC):
    ...
    def get_lr(self, step, nowarn=False):
        ...
        if self.t_total < 0:
            return 1.
        # print(f'step={step}, t_total={self.t_total}')
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                lr_scheduled = group['lr']
                # print(f'lr_scheduled0={lr_scheduled}')
                lr_scheduled *= group['schedule'].get_lr(state['step'])
                # print(f'lr_scheduled1={lr_scheduled}')  # 0

                update_with_lr = lr_scheduled * update
                # print(f'lr_scheduled2={lr_scheduled}')
...
class AdamW(Optimizer):
    ...
    def __init__(self,
        ...
        if not 0.0 <= betas[0] < 1.0:
            raise ValueError("Invalid beta parameter: {} - should be in [0.0, 1.0[".format(
                betas[0]))
